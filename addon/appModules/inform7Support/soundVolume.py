...
def scaledSoundPath(source, volume):
    ...
    if _cacheDirectory is None:
        ...
        atexit.register(_cacheDirectory.cleanup)
    stat = source.stat()
    key = f"{source.resolve()}:{stat.st_mtime_ns}:{stat.st_size}:{volume}"
    path = Path(_cacheDirectory.name) / \
        (hashlib.sha256(key.encode("utf-8")).hexdigest() + ".wav")
    if path.is_file():
        return path
    ...
    for offset in range(0, len(data), width):
        ...
        sample = int.from_bytes(
            data[offset: offset + width], "little", signed=width != 1)
        if width == 1:
            sample -= 128
        ...
        result[offset: offset +
               width] = sample.to_bytes(width, "little", signed=width != 1)
