...
FORMAT = {"reportFontSize": True, "reportColor": True,
...
class FakeScintillaTextInfo:
    ...
    def __init__(self, parts, start=0, end=None):
        self.data = b""
        self.styles = []
        self.boundaries = [0]
    ...
    def getTextWithFields(self, formatConfig=None):
        ...
        while pos < self._endOffset:
            ...
            items.extend([FieldCommand("formatChange", field),
...
class EditorTests(unittest.TestCase):
    def setUp(self):
        ...
        settings = SimpleNamespace(
            SettingsPanel=FakePanel, NVDASettingsDialog=SimpleNamespace(
                categoryClasses=self.categories)
        )

        def labeledControl(label, control, **kwargs):
            item = control(None, **kwargs)
            item.label = label
            return item

        helper = SimpleNamespace(
            BoxSizerHelper=lambda *a, **k: SimpleNamespace(
                addItem=lambda item: item,
                addLabeledControl=labeledControl,
            )
        )
        self.wx = SimpleNamespace(
            Choice=FakeChoice, VERTICAL=1, StaticBoxSizer=Mock(return_value=object()))
        controls = SimpleNamespace(EnhancedInputSlider=FakeSlider)
        gui = SimpleNamespace(settingsDialogs=settings,
                              guiHelper=helper, nvdaControls=controls)
        stubs = {
            "speech.commands": SimpleNamespace(WaveFileCommand=FakeWaveFileCommand),
            "config": SimpleNamespace(conf=self.conf),
            "colors": SimpleNamespace(
                RGB=SimpleNamespace(fromCOLORREF=lambda n: (
            ...
            "NVDAObjects.window.scintilla": SimpleNamespace(
                Scintilla=object, ScintillaTextInfo=FakeScintillaTextInfo
            ),
        ...
        self.paths = patch.object(
            sys, "path", [str(ROOT / "addon"), *sys.path])
        self.paths.start()
        ...
        self.Info = type(
            "TestInfo", (self.editor.InformSourceTextInfo,), {
                "_send": FakeScintillaTextInfo._send}
        )

    def info(self, parts, start=0, end=None):
        return self.Info(parts, start, end)

    def events(self, info, formatConfig=None):
        return [
            (action, kind)
    ...
    def spoken(self, info, unit="line", reason="caret", formatConfig=None):
        if unit is not None:
            info.expand(unit)
        sequence = []
        cache = {}
    ...
    def test_default_profile_values_and_runtime_toggle(self):
    ...
    def test_settings_apply_cancel_and_accessible_labels(self):
        panel = self.plugin.Inform7Settings()
        panel.makeSettings(object())
        self.assertEqual(panel.title, "Inform 7")
        self.assertEqual(
            self.wx.StaticBoxSizer.call_args.kwargs["label"], "Source editor")
        self.assertEqual(panel.syntaxModeChoice.label,
                         "Syntax highlighting feedback")
        self.assertEqual(panel.syntaxModeChoice.choices, [
                         "None", "Speech", "Speech and sounds", "Sounds"])
    ...
    def test_modes_preserve_legacy_profiles_and_explicit_modes_take_precedence(self):
        self.assertIn("default='legacy'",
                      self.conf.spec["inform7"]["syntaxFeedbackMode"])
        self.conf["inform7"] = {
            "syntaxFeedbackMode": "legacy", "automaticallyReadSyntaxHighlighting": False}
    ...
    def test_volume_accessible_label_apply_cancel_and_profiles(self):
        self.assertEqual(
            self.conf.spec["inform7"]["soundVolume"], "integer(min=0, max=100, default=100)")
        panel = self.plugin.Inform7Settings()
        panel.makeSettings(object())
        control = panel.soundVolumeControl
        self.assertEqual(control.label, "Syntax sound volume (%)")
        self.assertEqual(
            (control.min, control.max, control.GetValue()), (0, 100, 100))
        control.SetValue(25)
        panel.onDiscard()
    ...
    def test_volume_zero_mutes_only_sounds_and_reduced_volume_uses_cached_wave(self):
        syntax = importlib.import_module("appModules.inform7Support.syntax")
        self.conf["inform7"].update(
            syntaxFeedbackMode="speechAndSounds", soundVolume=0)
        self.assertEqual(syntax.renderMarker(
            ("begin", "quoted text")), ["Begin quoted text"])
        self.conf["inform7"]["syntaxFeedbackMode"] = "sounds"
        ...
        self.assertNotEqual(Path(first.fileName),
    ...
    def test_volume_scaling_preserves_pcm_format_duration_source_and_queued_files(self):
        ...
        with tempfile.TemporaryDirectory() as folder:
            for width in (1, 2, 3, 4):
                ...
                samples = [40, -40, 0, 80]
                data = b"".join(
                    (n + 128 if width == 1 else n).to_bytes(width,
                                                            "little", signed=width != 1)
                    for n in samples
                )
                ...
                values = [
                    int.from_bytes(frames[i: i + width],
                                   "little", signed=width != 1)
                ...
                self.assertEqual(
                    volumeModule.scaledSoundPath(source, 100), source)
                louder = volumeModule.scaledSoundPath(source, 50)
                self.assertNotEqual(quiet, louder)
                self.assertTrue(quiet.is_file())
                self.assertEqual(
                    volumeModule.scaledSoundPath(source, 25), quiet)

    def test_scaling_failure_never_plays_full_volume_and_keeps_speech(self):
        syntax = importlib.import_module("appModules.inform7Support.syntax")
        self.conf["inform7"].update(
            syntaxFeedbackMode="speechAndSounds", soundVolume=25)
        with patch.object(syntax, "scaledSoundPath", side_effect=wave.Error("invalid WAV")):
            self.assertEqual(syntax.renderMarker(
                ("begin", "quoted text")), ["Begin quoted text"])
        self.log.warning.assert_called_once()

    def test_sound_modes_queue_commands_at_existing_boundaries(self):
        info = self.info([('"hi ', 1), ("[name]", 2), ('"', 1)])
        self.conf["inform7"]["syntaxFeedbackMode"] = "sounds"
        sequence = self.spoken(info)
        self.assertEqual(
            [
                Path(item.fileName).name if isinstance(
                    item, FakeWaveFileCommand) else item
        ...
        for mode in ("speech", "none"):
            ...
            self.assertFalse(any(isinstance(item, FakeWaveFileCommand)
                             for item in self.spoken(info)))

    def test_all_supplied_sounds_are_mapped_and_character_reads_have_no_sound(self):
        syntax = importlib.import_module("appModules.inform7Support.syntax")
        self.conf["inform7"]["syntaxFeedbackMode"] = "sounds"
        for kind, prefix in (
                ("quoted text", "quote"),
            ...
            for action, suffix in (("begin", "start"), ("end", "end")):
                ...
                self.assertEqual(
                    Path(seq[0].fileName).name, f"{prefix}_{suffix}.wav")
    ...
    def test_missing_sound_retains_speech_in_combined_mode_and_logs_once(self):
        syntax = importlib.import_module("appModules.inform7Support.syntax")
        self.conf["inform7"]["syntaxFeedbackMode"] = "speechAndSounds"
    ...
    def test_inform6_reuses_comment_cues_with_its_own_spoken_label(self):
        syntax = importlib.import_module("appModules.inform7Support.syntax")
        self.conf["inform7"]["syntaxFeedbackMode"] = "speechAndSounds"
        for action, suffix, label in (("begin", "start", "Begin"), ("end", "end", "End")):
            sequence = syntax.renderMarker((action, "Inform 6 code"))
            self.assertEqual(
                Path(sequence[0].fileName).name, f"comment_{suffix}.wav")
            self.assertEqual(sequence[1], f"{label} Inform 6 code")

    def test_sound_boundary_order_survives_word_reading_and_adjacent_say_all_chunks(self):
        self.conf["inform7"]["syntaxFeedbackMode"] = "sounds"
        parts = [('"é ', 1), ("[name]", 2), ('"', 1)]
        whole = self.info(parts)

        def cues(info, unit="readingChunk", reason="sayAll"):
            return [
        ...
        for split in whole.boundaries[1:-1]:
            self.assertEqual(cues(self.info(parts, 0, split)) +
                             cues(self.info(parts, split)), expected)
        self.assertEqual(
            cues(self.info(parts, 4, 10), "word", "caret"), [
                "substitution_start.wav", "substitution_end.wav"]
        )

    def test_category_reload_and_late_termination(self):
    ...
    def test_sizes_colours_and_disabled_syntax(self):
        self.conf["inform7"]["automaticallyReadSyntaxHighlighting"] = False
        ...
        field, _ = info._getFormatFieldAndOffsets(
            0, {"reportFontSize": False, "reportColor": False})
        self.assertEqual(field, {})

    def test_say_is_plain_and_markers_surround_delimiters(self):
    ...
    def test_style_mask_and_all_categories(self):
    ...
    def test_nested_and_adjacent_comments(self):
        ...
        self.assertEqual(
            self.events(self.info([("[a][b]", 5)])), [
                ("begin", "comment"), ("end", "comment")] * 2
        )

    def test_partial_and_multiline_ranges(self):
        parts = [('"one\ntwo"', 1), (".", 0)]
        self.assertEqual(self.events(self.info(parts, 2, 4)), [])
        self.assertEqual(self.events(self.info(parts, 0, 4)),
                         [("begin", "quoted text")])
        self.assertEqual(self.events(self.info(parts, 5, 9)),
                         [("end", "quoted text")])
        self.assertEqual(self.events(self.info(parts, 5, 8)), [])

    def test_adjacent_substitutions_and_inform6_spans(self):
        for parts, kind in (
            ...
            self.assertEqual([e for e in expected if e[1] == kind], [
                             ("begin", kind), ("end", kind)] * 2)
            for split in whole.boundaries[1:-1]:
                self.assertEqual(
                    self.events(self.info(parts, 0, split)) +
                    self.events(self.info(parts, split)), expected
                )

    def test_quote_can_end_substitution_and_outer_quote_together(self):
        self.assertEqual(
    ...
    def test_adjacent_chunks_equal_whole_read_without_cache_deduplication(self):
    ...
    def test_unfinished_eof_never_invents_closures(self):
        for parts, ends in (
                ([('"unfinished', 1)], []),
                ([('"', 1)], []),
                ([("say ", 0), ('"', 1)], []),
            ...
            with self.subTest(parts=parts):
                self.assertEqual([e for e in self.events(
                    self.info(parts)) if e[0] == "end"], ends)

    def test_complete_eof_and_utf8(self):
        for text, style, kind in (
            ...
            self.assertEqual(self.events(self.info([(text, style)])), [
                             ("begin", kind), ("end", kind)])
        ...
        for split in whole.boundaries[1:-1]:
            self.assertEqual(
                self.events(self.info(parts, 0, split)) +
                self.events(self.info(parts, split)),
                self.events(whole),
            )

    def test_character_typing_and_formatting_queries_are_silent(self):
        info = self.info([('"hi"', 1)])
    ...
    def test_word_reading_announces_entry_and_exit_at_actual_boundaries(self):
        parts = [('"hello ', 1), ("[name]", 2), (' friend"', 1), (".", 0)]
        for start, end, expected in (
                (0, 7, ["Begin quoted text", '"hello ']),
                (7, 13, ["Begin text substitution",
                 "[name]", "End text substitution"]),
        ...
        self.assertEqual(self.spoken(
            self.info(parts, 7, 13), "word"), ["[name]"])

    def test_character_delimiters_keep_nvda_single_character_spelling_path(self):
        parts = [('"', 1), ("[x]", 2), ('"', 1), (" ", 0), ("[c]", 5)]
        for start in (0, 1, 3, 4, 6, 8):
            for expanded in (False, True):
                ...
                while fields and not isinstance(fields[0], str):
                    self.assertNotIn(self.editor.SYNTAX_FIELD,
                                     fields.pop(0).field)
                self.assertEqual(fields, [info.text])

    def test_word_closing_delimiter_keeps_marker_after_copy(self):
        info = self.info([('"hi"', 1)], 3, 4)
        info.expand("word")
        info = info.copy()
    ...
    def test_syntax_ignores_detect_format_after_cursor_but_preserves_text(self):
        info = self.info([("say ", 0), ('"hi ', 1), ("[name]", 2), ('".', 1)])
        for detect in (False, True):
            fields = info.getTextWithFields(
                FORMAT | {"detectFormatAfterCursor": detect})
            self.assertEqual(
                "".join(item for item in fields if isinstance(item, str)), info.text)
            self.assertEqual(
                len(self.events(info, FORMAT | {"detectFormatAfterCursor": detect})), 3)
    ...
    def test_renderer_supports_speech_commands_and_retains_normal_speech(self):
        info = self.info([('"hi"', 1)])
        command = object()
        with patch.object(self.editor, "renderMarker", return_value=[command]):
            seq = info.getFormatFieldSpeech(
                Field(speakNormal=True, **
                      {self.editor.SYNTAX_FIELD: (("begin", "quoted text"),)}),
                unit="line",
            )
        self.assertEqual(seq, ["normal formatting", command])

    def test_failure_falls_back_without_source_in_log(self):
        info = self.info([('"private source"', 1)])
        with patch.object(info, "_syntaxEvents", side_effect=RuntimeError("private source")):
            self.assertEqual(self.events(info), [])
            self.assertEqual(
                "".join(x for x in info.getTextWithFields() if isinstance(x, str)), info.text)
        self.assertNotIn("private source", str(self.log.mock_calls))

    def test_style_sampling_is_local_to_requested_range(self):
        info = self.info([("a" * 10000, 0), ('"hi"', 1),
                         ("b" * 10000, 0)], 10000, 10004)
        self.events(info)
        positions = [offset for message,
                     offset in info.queries if message == 2010]
